...
def energy_cost_per_month(car) -> float:
    ...
    if vehicle_type in ("EV", "BEV", "ELECTRIC"):
        kwh_per_month = (to_num(getattr(car, "consumption_kwh_per_100km", 0.0)) / 100.0) * monthly_km
    ...
    price = prices["diesel_price_sek_litre"] if vehicle_type == "DIESEL" else prices["bensin_price_sek_litre"]
