...
from backend.models.models import (
    PriceSettings,
)  # absolute import to avoid relative hops
...
def normalize_prices(ps: PriceSettings | None) -> dict[str, Any]:
    ...
    loss_pct = (
        getattr(ps, "charging_loss_pct", DEFAULTS["charging_loss_pct"])
        or DEFAULTS["charging_loss_pct"]
    )
    ...
    return {
        ...
        "tire_lifespan_years": max(
            1,
            int(
                _pos(
                    getattr(ps, "tire_lifespan_years", None),
                    DEFAULTS["tire_lifespan_years"],
                )
            ),
        ),
        "downpayment_sek": float(
            getattr(ps, "downpayment_sek", DEFAULTS["downpayment_sek"]) or 0.0
        ),
        "interest_rate_pct": float(
            getattr(ps, "interest_rate_pct", DEFAULTS["interest_rate_pct"]) or 0.0
        ),
    }


def amortized_totals(
    purchase_price: float, downpayment_sek: float, interest_rate_pct: float, years: int
) -> tuple[float, float]:
    """
