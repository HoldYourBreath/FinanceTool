...
export default function PastMonths() {
  ...
  useEffect(() => {
    async function fetchPastMonths() {
      try {
        ...
        const pastMonths =
          currentIndex > 0 ? months.slice(0, currentIndex) : [];
  ...
  if (loading)
    return <div className="text-center p-4">Loading Past Months...</div>;
    ...
    if (type === "income") {
      return {
        "Janne Income": items.filter(
          (i) => i.name.includes("Janne") && !i.name.includes("Rent"),
        ),
        "Kristine Income": items.filter(
          (i) => i.name.includes("Kristine") && !i.name.includes("Rent"),
        ),
  ...
  return (
    <div>
      <h3 className="text-lg font-semibold mb-2">
        {title} (Total {total.toLocaleString()} SEK)
      </h3>
        ...
        ([category, items]) =>
          items.length > 0 && (
            <div key={category} className="mb-2">
              <div
                className={`font-bold ${categoryColors[category] || "text-gray-700"}`}
              >
