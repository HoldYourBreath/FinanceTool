...
const isChildcare = (s) =>
  /\b(day ?care|child ?care|childcare|preschool|förskola|dagis|barnomsorg)\b/i.test(String(s || ''));
...
export default function MonthlyOverview() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const futureMonths = currentIndex >= 0 ? months.slice(currentIndex) : months;
  ...
  if (!Array.isArray(monthsData) || monthsData.length === 0) {
    return <div className="text-center p-4 text-red-600">No Monthly Data Available.</div>;
  }

  const categorizeIncome = (incomes) => {
    const categories = {
      'Janne Income': incomes.filter(inc => inc.name.includes('Janne') && !inc.name.includes('Rent')),
      'Kristine Income': incomes.filter(inc => inc.name.includes('Kristine') && !inc.name.includes('Rent')),
...
const categorizeExpense = (expenses) => {
  ...
  expenses.forEach((exp) => {
    ...
    if (isChildcare(d)) {
      ...
      hasWord(d, 'car') || hasWord(d, 'transport') ||
      hasWord(d, 'diesel') || hasWord(d, 'fuel') ||
      hasWord(d, 'parking') || hasWord(d, 'tire') || hasWord(d, 'tyre')
    ) {
...
const planned = purchases.filter((p) => {
  ...
  return m.name === `${purchaseDate.getFullYear()}-${String(purchaseDate.getMonth() + 1).padStart(2, '0')}`;
});
            ...
            {rowMonths.map((month) => {
              ...
              return (
                <div key={month.id} className="w-[320px] border p-4 rounded-lg shadow space-y-4 bg-white">
                  ...
                  {/* Incomes Section */}
                  <div>
                    <h3 className="text-lg font-semibold mb-2">💰 Income (Total {totalIncome.toLocaleString()} SEK)</h3>
                    {Object.entries(incomeCategories).map(([category, items]) => {
                      ...
                      return (
                        <div key={category} className="mb-2">
                          <div className={`font-bold ${color}`}>{category} — {total.toLocaleString()} SEK</div>
                          {items.map((inc, idx) => (
                            <div key={idx} className="flex justify-between text-sm">
                              <span>{inc.name}</span>
                              <span>{Number(inc.amount).toLocaleString()} SEK</span>
                            </div>
                          ))}
                        </div>
                      );
                  ...
                  {/* Expenses Section */}
                  <div>
                    <h3 className="text-lg font-semibold mb-2">💸 Expenses (Total {totalExpenses.toLocaleString()} SEK)</h3>
                    {Object.entries(expenseCategories).map(([category, items]) => {
                      if (items.length === 0) return null;
                        ...
                        <div key={category} className="mb-2">
                          ...
                          {items.map((exp, idx) => (
                            <div key={idx} className="flex justify-between text-sm">
                              ...
                              <span>- {Number(exp.amount).toLocaleString()} SEK</span>
                            </div>
                          ))}
                        </div>
                      );
                ...
                {month.loanAdjustments && month.loanAdjustments.length > 0 && (
                ...
                <div className="mt-4 border-t pt-2 text-sm text-gray-700 space-y-1">
                  <h3 className="text-base font-semibold flex items-center gap-2">💰 Funds</h3>
                  <div className="flex justify-between">
                    <span>Start:</span>
                    <span>{Number(month.startingFunds).toLocaleString('sv-SE')} SEK</span>
                  </div>
                  <div className="flex justify-between">
                    <span>End:</span>
                    <span>{Number(month.endingFunds).toLocaleString('sv-SE')} SEK</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Loan Remaining:</span>
                    <span>
                      {Number(month.loanRemaining || 0).toLocaleString('sv-SE')} SEK
                    </span>
                  </div>
                </div>
