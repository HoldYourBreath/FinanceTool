// src/components/car-evaluation/CarsTable.jsx
import * as carCalc from "../../utils/carCalc";
...
// prefer any available recalculator
const recalc = carCalc.recalcRow || carCalc.recalcOne || ((c) => c);
...
export default function CarsTable({
  ...
  // 🔁 Always re-derive rows with current prices (instant UI reaction)
  const recalcedCars = useMemo(
    () => cars.map((c) => recalc(c, prices)),
    [cars, prices]
  );

  // 🧰 normalize keys your table expects (fallback to server persisted)
  const normalizedCars = useMemo(
    () =>
      recalcedCars.map((c) => ({
        ...c,
        // unify consumption key
        consumption_kwh_100km:
          c.consumption_kwh_100km ?? c.consumption_kwh_per_100km ?? null,
        consumption_kwh_per_100km:
          c.consumption_kwh_per_100km ?? c.consumption_kwh_100km ?? null,
        // unify TCO keys
        tco_total_3y: c.tco_total_3y ?? c.tco_3_years ?? null,
        tco_total_5y: c.tco_total_5y ?? c.tco_5_years ?? null,
        tco_total_8y: c.tco_total_8y ?? c.tco_8_years ?? null,
        // if per-month not provided, compute from totals
        tco_per_month_3y:
          c.tco_per_month_3y ??
          (Number.isFinite(c.tco_total_3y) ? Math.round(c.tco_total_3y / 36) : null),
        tco_per_month_5y:
          c.tco_per_month_5y ??
          (Number.isFinite(c.tco_total_5y) ? Math.round(c.tco_total_5y / 60) : null),
        tco_per_month_8y:
          c.tco_per_month_8y ??
          (Number.isFinite(c.tco_total_8y) ? Math.round(c.tco_total_8y / 96) : null),
      })),
    [recalcedCars]
  );

  // value getter with unified consumption key + ICE handling
  const getVal = (car, key) => {
    ...
    if (EV_ONLY_KEYS.includes(key) && (n === 0 || raw === 0 || raw === "0")) {
      return null; // treat 0 as N/A for EV-only fields
    }
  ...
  const sortedCars = useMemo(() => {
    ...
    return list;
  }, [normalizedCars, sortBy, sortDir]);

  // editing in-row → update source array, then re-derive that row immediately
  const onChange = (carId, field, value) => {
    setCars((prev) => {
      ...
      if (field === "consumption_kwh_100km" || field === "consumption_kwh_per_100km") {
        patch = { consumption_kwh_100km: parsed, consumption_kwh_per_100km: parsed };
      }

      next[i] = recalc({ ...next[i], ...patch }, prices);
      return next;
    });
  };
      ...
      <section className="rounded-2xl bg-cyan-50/70 ring-1 ring-cyan-200 p-3 shadow-sm backdrop-blur-sm">
        <div className="max-h-[72vh] overflow-auto rounded-lg ring-1 ring-white/50">
          <table className="w-full table-auto border-separate border-spacing-0">
            <thead>
              <tr>
                <Header label="Car Model" sortKey="model" align="text-left" sortBy={sortBy} sortDir={sortDir} onSort={onSort}
                ...
                <Header label={"Consumption\n(kWh/100km)"} sortKey="consumption_kwh_100km" sortBy={sortBy} sortDir={sortDir} onSort={onSort} />
                ...
                <Header label={"WLTP EV\nRange (km)"} sortKey="range_km" sortBy={sortBy} sortDir={sortDir} onSort={onSort} />
