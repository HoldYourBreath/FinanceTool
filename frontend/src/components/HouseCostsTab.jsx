    ...
    return (
      <div className="w-full md:w-1/2 p-4">
            ...
            {data.map((item) => (
                ...
                <td className="pl-2 flex items-center justify-between min-w-[130px]">
                    ...
                    onClick={() => {
                        ...
                        if (confirmDelete) {
                        handleDelete(title.includes('House') ? 'house' : 'land', item.id);
                        }
                    }}
        ...
        <div className="mt-2 text-sm">
          <div><strong>Total:</strong> {formatSEK(total)}</div>
  ...
  return (
    <div className="p-4 bg-white rounded shadow">
      ...
      <div className="flex flex-col md:flex-row">
        {renderSection('🏠 House Payments', houseCosts, renderAddForm('house', newHouse, setNewHouse))}
        {renderSection('🌱 Land Payments', landCosts, renderAddForm('land', newLand, setNewLand))}
      </div>
      <div className="mt-4 text-sm">
        <strong>House Build Progress:</strong><br />
        Items Completed: {completedItems} / {totalItems}<br />
        Cost Completed: {formatSEK(doneAmount)} / {formatSEK(totalAmount)}<br />
